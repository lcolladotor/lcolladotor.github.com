...
output:
  knitrBootstrap::bootstrap_document:
    ...
    theme: cerulean # slate # cyborg slate
...
paginate: 3 # it lets you choose how many posts to show per page (Jekyll supports pagination);
# type: post # ??
...
%\VignetteIndexEntry{bootstrap format}
-->
...
<div class="prev_next post_nav">
    ...
    <p class="previous">
        «&nbsp;<a href="{{ page.previous.url }}/" rel="previous" title="{{ page.previous.title }}">{{ page.previous.title | truncatewords: 15 }}</a>
    </p>
    {% endif %}
    {% if page.next.url %}
    <p class="next">
        <a href="{{ page.next.url }}/" rel="next" title="{{ page.next.title }}">{{ page.next.title | truncatewords: 15 }}</a>&nbsp;»
    </p>
    {% endif %}
</div>
...
<!--break-->
[Name of Link]({% post_url 2014-12-29-dotsPkgCopy %})
<!--break-->
[site.categories.CATEGORY]({% site.categories.CATEGORY %})
<p>
 {{% site.categories.CATEGORY %}}
<p>site.time {{ site.time | date_to_xmlschema | markdownify }}
<p>{{ site.time | markdownify  | date_to_xmlschema }}
<p>site.production_url{{ site.production_url | markdownify}}{{ site.rss_path| markdownify }}
        <title>{{ site.title| markdownify }}</title>


{% for post in paginator.posts %}
   <!-- here add you post markup -->
   <h1><a href="{{ post.url | markdownify }}">{{ post.title| markdownify }}</a></h1>
   <p class="author">
    <span class="date">{{post.date | markdownify| date: "%Y-%m-%d"}}</span>
  </p>
  <div class="content">
...
\pagebreak
# Reproducibility
This vignette was produced via:
...
```{r reproducibility2, echo=FALSE, bootstrap.show.code=FALSE, message=FALSE, warning = FALSE, results='hide'} 
